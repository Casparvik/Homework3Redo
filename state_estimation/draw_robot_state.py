...
class DrawRobotAndWalls(QWidget):
    ...
    def draw_robot_gauss(self, qp):
        ...
        SD = self.state_estimation.SD
        dy = [ np.exp(-np.power(mu - x, 2.0) / (2 * np.power(SD, 2.0))) for x in dx]
        pts = []
        max_y = np.max(dy)
    ...
    def draw_move_gauss(self, qp):
        ...
        dx = np.linspace(0,1,200)
        mu = self.robot_state.robot_loc
        SD = self.robot_state.robot_move_SD_err
        dy = [ np.exp(-np.power(mu - x, 2.0) / (2 * np.power(SD, 2.0))) for x in dx]
        pts = []
        max_y = np.max(dy)
        height = 0.2
...
class StateEstimationGUI(QMainWindow):
    ...
    def set_gauss(self):
        ...
        self.robot_scene.world_state.set_wall_SD( self.prob_see_wall_SD.value() )
        self.repaint()
